...
Date         : 2025-03-02 12:10:58
LastEditors  : 解博炜 8894423+xie-bowei@user.noreply.gitee.com
LastEditTime : 2025-03-02 12:26:06
FilePath     : /piper_hci/utils/Rbf_sw.py
Description  : 用滑动窗口模式做一个demo，处理简单的3D曲线
version      : 
notion       : 
'''
import numpy as np
...
class Rbf_sw:
    def __init__(self, s:int, d:int, r=1):
        self.s = s
        self.d = d
        self.window = collections.deque(maxlen=s)
...
if __name__ == '__main__':
    ...
    rbf_sw = Rbf_sw(s=15, d=3)
    smoothed_x_values, smoothed_y_values, smoothed_z_values = [], [], []

    for i in range(len(t_values)):
        rbf_sw.add_point(x_values[i], y_values[i], z_values[i])
